// app/api/admin/local-authorities/[laId]/mutate/route.ts
...
/* ---------- GET: quick probe to avoid 404 confusion ---------- */
export async function GET(_req: Request, { params }: { params: { laId: string } }) {
  return NextResponse.json({ ok: true, dataset: "mutate", laId: params.laId });
}

/* ---------- POST: main mutate handler ---------- */
export async function POST(req: Request, { params }: { params: { laId: string } }) {
  ...
  const laId = String(params.laId);               // <— IMPORTANT: matches folder name [laId]
